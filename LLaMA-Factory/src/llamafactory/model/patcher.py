...
def patch_config(
    ...
    if "InternVLChatModel" in (getattr(config, "architectures", None) or []):
        raise ValueError(
    ...
    if "LlavaLlamaForCausalLM" in (getattr(config, "architectures", None) or []):
...
def patch_model(
    ...
    # 如果存在 LoRA 适配器，先不要扩展词表，避免在加载适配器前因尺寸不匹配报错；
    # 等适配器加载/合并完成后再扩展（在 load_model 中追加一步）。
    if model_args.resize_vocab and (not getattr(model_args, "adapter_name_or_path", None)):
        resize_embedding_layer(model, tokenizer)
    elif model_args.resize_vocab and getattr(model_args, "adapter_name_or_path", None):
        logger.info_rank0("Detected adapters; postpone resizing embeddings until after adapter initialization.")
