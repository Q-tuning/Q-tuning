...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def _resolve_last_layer_module(self, model: "torch.nn.Module"):
        """
        返回最后一个解码层模块（与 _resolve_last_attention_module 相同路径查找），便于注册 hook 捕获最后一层隐状态，
        避免在 forward 返回中打开 hidden_states。
        """
        try:
            core_model = model.module if hasattr(model, "module") else model
            base = getattr(core_model, "get_base_model", lambda: core_model)()
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        score_last_attn = None
        score_last_hidden = None
        if self.token_method in ["sample_fastv", "sample_sparsevlm"]:
            # 恢复 hook 方案：绕过 Accelerate 包装，避免返回结构中出现大张量被转 FP32
            core_model = model.module if hasattr(model, "module") else model
            last_layer = self._resolve_last_layer_module(core_model)
            attn_mod = self._resolve_last_attention_module(core_model)
            saved: dict[str, torch.Tensor] = {}

            def _attn_hook(_mod, _inp, out):
                # out 可能是 (attn_output, attn_weights, ...) 或直接 attn_weights
                tensor = out[1] if (isinstance(out, tuple) and len(out) > 1) else out
                if isinstance(tensor, torch.Tensor):
                    saved["attn"] = tensor

            def _hidden_hook(_mod, _inp, out):
                tensor = out[0] if (isinstance(out, tuple) and len(out) > 0) else out
                if isinstance(tensor, torch.Tensor):
                    saved["hidden"] = tensor

            h1 = attn_mod.register_forward_hook(_attn_hook) if attn_mod is not None else None
            h2 = last_layer.register_forward_hook(_hidden_hook) if (self.token_method == "sample_sparsevlm" and last_layer is not None) else None
            try:
                with torch.no_grad():
                    _ = core_model(
                        ...
                        return_dict=False,
                        output_hidden_states=False,  # 由 hook 捕获
                        output_attentions=True,      # 需要内部计算注意力以便 hook 捕获
                    )
                score_last_attn = saved.get("attn", None)
                score_last_hidden = saved.get("hidden", None)
            finally:
                if h1 is not None:
                    h1.remove()
                if h2 is not None:
                    h2.remove()
            if isinstance(score_last_attn, torch.Tensor) and score_last_attn.dim() == 3:
                score_last_attn = score_last_attn.unsqueeze(1)

        # Forward pass with target model
        need_attn = self.token_method in {"sample_fastv", "sample_sparsevlm"}
        need_hid = self.token_method == "sample_sparsevlm"
            ...
            if remaining_drop_for_Q2 > 0 and self.token_ratio!=1.0:
                ...
                if len(Q2_in_kept) > 0:
                    ...
                    if self.token_method == "rho" and hasattr(self, "reference_model") and self.reference_model is not None:
                        ...
                        # s_i = (1 - lambda) * ppl_i + lambda * (ppl_left + ppl_right)
                        # 仅使用左右相邻各一个token，且仅在有效 label 区域统计
                        wise_lambda = float(self.wise_lambda)
                        # print(f"wise_lambda: {wise_lambda}")
                        # ppl per token ≈ exp(CE), mask to valid region
                        ...
                        s_wise = (1.0 - wise_lambda) * ppl_token + wise_lambda * (left + right)
                        scores_kept = -(s_wise * base_valid_mask)
                    elif self.token_method == "loss":
                        scores_kept = (ce_token_loss_raw * base_valid_mask)  # fallback: loss-based
                    elif self.token_method == "random":
                        # 随机为每个token分配一个分数（仅在有效token区域）
                        rand_scores = torch.rand_like(ce_token_loss_raw)  # shape与ce_token_loss_raw一致
                        scores_kept = rand_scores * base_valid_mask
        ...
        if self.token_method == "random": 
            ...
            # selection path (non-wisely plug): keep top token_ratio by wise score within B'
            valid = allowed_mask_b.to(dtype=target_token_loss.dtype)
            per_token_loss = target_token_loss[idx_sub]
            wise_lambda = float(getattr(self.args, "wise_lambda", 0.5) or 0.5)
            ppl = torch.exp(per_token_loss) * valid
            left = F.pad(ppl[:, :-1], (1, 0)) * F.pad(valid[:, :-1], (1, 0))
            right = F.pad(ppl[:, 1:], (0, 1)) * F.pad(valid[:, 1:], (0, 1))
            score_2d = (1.0 - wise_lambda) * ppl + wise_lambda * (left + right)
            ...
            # select top-k tokens by per-token loss within valid positions in B'
            flat_scores = target_token_loss[idx_sub].reshape(-1)
            valid_indices = torch.nonzero(allowed_mask_b.reshape(-1) > 0, as_tuple=True)[0]
            selection_mask_flat = torch.zeros_like(flat_scores)
            if len(valid_indices) > 0:
                k_sel = max(1, int(round(len(valid_indices) * self.token_ratio)))
            ...
            # 使用 no_grad 评分前向提取的最后一层注意力，避免训练前向返回巨大张量
            last_attn = score_last_attn   # [B, H, Lq, Lk] 或 [B, Lq, Lk] 已在上游标准化
            if last_attn is None or not isinstance(last_attn, torch.Tensor):
                raise RuntimeError("sample_fastv 需要注意力权重，但未获取到 attentions。请确认当前模型支持 output_attentions 并已启用。")
            if last_attn.dim() == 3:
                last_attn = last_attn.unsqueeze(1)
            if last_attn.dim() != 4:
                raise RuntimeError(f"sample_fastv 期望注意力形状为 4D，但得到 {last_attn.dim()}D。实际 shape={tuple(last_attn.shape)}")
            ...
            last_attn = score_last_attn   # [B, H, Lq, Lk] 或 [B, Lq, Lk]
            last_hidden = score_last_hidden  # [B, L, D]
            if last_attn is None or not isinstance(last_attn, torch.Tensor):
                raise RuntimeError("sample_sparsevlm 需要注意力权重，但未获取到 attentions。")
            if last_hidden is None or not isinstance(last_hidden, torch.Tensor):
                raise RuntimeError("sample_sparsevlm 需要最后一层隐状态，但未获取到 hidden_states。")
            if last_attn.dim() == 3:
                last_attn = last_attn.unsqueeze(1)
            if last_attn.dim() != 4:
                raise RuntimeError(f"sample_sparsevlm 期望注意力形状为 4D，但得到 {last_attn.dim()}D。实际 shape={tuple(last_attn.shape)}")
            ...
            P_mean = last_attn_sub.mean(dim=(0, 1)).float()  # [Lq, Lk]
            try:
