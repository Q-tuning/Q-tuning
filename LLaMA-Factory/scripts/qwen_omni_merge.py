...
from transformers import AutoModel, AutoProcessor, AutoTokenizer, Qwen2_5OmniThinkerForConditionalGeneration  # type: ignore
...
def merge_lora(
    ...
    # Ensure token embedding size matches tokenizer vocab size
    tokenizer = None
    # Prefer tokenizer from LoRA directory if available (may include added_tokens.json)
    try:
        tokenizer = AutoTokenizer.from_pretrained(lora_checkpoint_path, use_fast=True)
    except Exception:
        try:
    ...
    if tokenizer is not None:
        ...
        if hasattr(model, "resize_token_embeddings"):
            try:
                ...
                # proceed even if resizing fails; merging may still work for modules without embeddings
                pass
...
def save_full_model(
    ...
    # except Exception:
    #     tokenizer = None
    # if hasattr(base_model, "resize_token_embeddings") and tokenizer is not None:
